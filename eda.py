import pandas as pd
df = pd.read_csv("data/total_migration_diff_first_100.csv")
...
for migration in migrations:
    ...
    for file in os.listdir(parsed1):
        ...
        for item in lst:
            ...
            level1_parsed1_info[item["class_mode"]] = level1_parsed1_info.get(item["class_mode"], 0) + 1
    for file in os.listdir(parsed2):
        with open(os.path.join(parsed2, file), "r") as f:
            lst = json.load(f)
        for item in lst:
            level1_parsed2_info[item["class_mode"]] = level1_parsed2_info.get(item["class_mode"], 0) + 1
    statistic_parsed1.append(level1_parsed1_info)
    statistic_parsed2.append(level1_parsed2_info)


class_parsed1 = pd.DataFrame(statistic_parsed1, columns=["Modified", "Renamed-Modified", "Added", "Deleted", "Unchanged", "Renamed-Unchanged"])
...
class_parsed2 = pd.DataFrame(statistic_parsed2, columns=["Modified", "Renamed-Modified", "Added", "Deleted", "Unchanged", "Renamed-Unchanged"])
...
for migration in migrations:
    ...
    for ver1_file_name in os.listdir(parsed1):
        ...
        for ver1_class in ver1_classes:
            ver1_class_modes[ver1_class["class_mode"]] = ver1_class_modes.get(ver1_class["class_mode"], 0) + 1 
            for method in ver1_class["methods"]:
                cnt_methods += 1
                ver1_method_modes[method["method_mode"]] = ver1_method_modes.get(method["method_mode"], 0) + 1 
    ...
    for ver2_file_name in os.listdir(parsed2):
        ...
        for ver2_class in ver2_classes:
            ver2_class_modes[ver2_class["class_mode"]] = ver2_class_modes.get(ver2_class["class_mode"], 0) + 1
            for ver2_method in ver2_class["methods"]:
                cnt_methods += 1
                ver2_method_modes[ver2_method["method_mode"]] = ver2_method_modes.get(ver2_method["method_mode"], 0) + 1
...
method_parsed1 = pd.DataFrame(statistic_parsed1, columns=["Modified", "Renamed-Modified", "Added", "Deleted", "Unchanged", "Renamed-Unchanged"])
...
method_parsed2 = pd.DataFrame(statistic_parsed2, columns=["Modified", "Renamed-Modified", "Added", "Deleted", "Unchanged", "Renamed-Unchanged"])
method_parsed2
