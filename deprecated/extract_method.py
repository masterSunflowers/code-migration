                        ...
                        for c in child.named_children:
                            if c.type == "modifiers":
                                ...
                                method_info["tree_path"] = (
                                    method_info["tree_path"] + ":" + method_info["name"]
                                )
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Parsing"):
        if not pd.isna(row["diff_files"]):
            ...
            for rev_path, tree, file_mode, commit_hash, map_path in parse_java_files(
                ...
                lst_class_info, lst_method_info = get_definitions(
                    rev_path, tree, file_mode, map_path
                )
                if commit_hash == row["prev_commit"]:
                    all_previous_class.extend(lst_class_info)
                    all_previous_method.extend(lst_method_info)
                else:
                    all_current_class.extend(lst_class_info)
                    all_current_method.extend(lst_method_info)

            prev_identifier = row["repoName"] + "--" + row["prev_commit"]
            if not os.path.exists(os.path.join(args.output_dir, prev_identifier)):
                os.makedirs(os.path.join(args.output_dir, prev_identifier))
            with open(
                os.path.join(args.output_dir, prev_identifier, "class_info.json"),
                "w",
            ) as f:
                json.dump(all_previous_class, f, indent=4)
            with open(
                os.path.join(args.output_dir, prev_identifier, "method_info.json"),
                "w",
            ) as f:
                json.dump(all_previous_method, f, indent=4)

            cur_identifier = row["repoName"] + "--" + row["endCommit"]
            if not os.path.exists(os.path.join(args.output_dir, cur_identifier)):
                os.makedirs(os.path.join(args.output_dir, cur_identifier))
            with open(
                os.path.join(args.output_dir, cur_identifier, "class_info.json"),
                "w",
            ) as f:
                json.dump(all_current_class, f, indent=4)
            with open(
                os.path.join(args.output_dir, cur_identifier, "method_info.json"),
                "w",
            ) as f:
                json.dump(all_current_method, f, indent=4)
